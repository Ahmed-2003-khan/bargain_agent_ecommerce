...
async def send_negotiation_outcome_to_db(
    ...
    try:
        ...
        logger.error("DB Sync HTTP Error [%s]: %s - Response: %s", session_id, e, e.response.text)
    except Exception as e:
        logger.error("Failed to send negotiation outcome to DB for session %s. Exception type: %s, Error: %s", session_id, type(e).__name__, str(e), exc_info=True)
...
async def chat_endpoint(
    ...
    try:
        ...
        async with state_manager.session_lock(redis_key):
            ...
            # Fire off DB save if deal was closed
            if brain_action in ("ACCEPT", "DEAL"):
                final_price = result.get("counter_price") or user_offer or 0.0
